#!/bin/sh
# Builds the Debian package from a self contained linux-x64 publish.
...
sed "s/__VERSION__/$VERSION/" linux/deb/control.in > "$STAGE/DEBIAN/control"
install -m 0755 linux/deb/preinst linux/deb/postinst linux/deb/prerm linux/deb/postrm "$STAGE/DEBIAN/"

mkdir -p "$OUT"
dpkg-deb --build --root-owner-group "$STAGE" "$OUT/ads-gamepad-service_${VERSION}_amd64.deb"
