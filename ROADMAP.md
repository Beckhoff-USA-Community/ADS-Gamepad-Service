...
* The GitHub package feed works as a TwinCAT Package Manager source with a personal access token, and installs from it were verified on a real controller. One limitation is inherent to GitHub: its feed does not implement the NuGet search service, so browsing and listing that feed show nothing or report a metadata error. Installs work normally when the package id is given, so the documented flow is tcpkg install with the full package id.
