...
| 2.12.0 | 1.4 | The Debian package is now also built for arm64, covering the ARM controllers such as the CX8200 and CX9240 series, and releases from this version on attach both packages; the TcCOM module gains an ARM build with 0.0.0.12; the current ARM kernel image additionally needs the raw HID layer module described on the Linux installation page; the service itself is unchanged |
